...
- (void)setStyleIndex:(NSInteger)index
{
    ...
    if (![picCountFlag isEqualToString:@""]) {
        ...
        _styleFileName = [NSString stringWithFormat:@"number_%@_style_%ld.plist",picCountFlag,_styleIndex];
        _styleDict = nil;

        NSBundle *bundle = [NSBundle bundleForClass:[MeituEditStyleViewController class]];
        NSURL *url = [[bundle resourceURL]URLByAppendingPathComponent:@"MeituDemo.bundle/MeituDemo.bundle"];
       NSBundle * bundleee = [NSBundle bundleWithURL:url];

        _styleDict = [NSDictionary dictionaryWithContentsOfFile:[bundleee.resourcePath stringByAppendingPathComponent:_styleFileName]];
        if (_styleDict) {
            [self resetAllView];
            [self resetStyle];
        }
